...
class Crypter(object):
	...
	def sha1(self,s):
		return str(sha1(s).hexdigest()).upper()

	def getSmonChecker(self,s,ts):
	...
	def EG_knlCurrentUpTime(self,ts):
		v5=0x5A8EF52B
		v1=ts-v5
		tsu=22222#randint(1,999999)
		return 1000 * v1 + ((274877907 * tsu >> 38) + ((274877907 * tsu) >> 63))

	def GetPlayerServerConnectElapsedTime(self,ts=None):
		if not ts:
			ts=int(time.time())
		return ((0x5A92AD2B + ((((self.EG_knlCurrentUpTime(ts) - 0xE86C29C) >> 3) * 0x20C49BA5E353F7CF >> 64) >> 4)) ^ 0x1C2F0688)
